...
class handle_model():
    ...
    def cross_entropy_with_weights(self, pred, y):
        ...
        #                                                  y=y_cpu.numpy())
        class_weights = [0.6, 0.4] #[1, 678.487]
...
class handle_ae_model():
    ...
    def run(self, epochs=5, learning_rate=1e-3, loss_fn=nn.CrossEntropyLoss(), loss_reconstruction=nn.MSELoss(), path_to_save=""):
        """
        Trains and tests the model on provided datasets for specified number of epochs.
        """
        self.batch_size = self.train_dataloader.batch_size
        ...
        # from nn_utilities import Lion
        # self.optimizer = Lion(self.model.parameters(), lr=learning_rate)
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, 'min', factor=0.1, patience=4,
                                                                    verbose=False)
        self.early_stopper = EarlyStopper(patience=8, min_delta=0.008)
        try:
            self.model_info = summary(self.model, input_size=(self.batch_size, 10))
    ...
    def train(self, dataloader):
        ...
        from torchmetrics.classification import BinaryAccuracy, BinaryPrecision, BinaryRecall, BinaryF1Score, \
            BinarySpecificity, BinaryAUROC, BinaryConfusionMatrix
        ...
        for batch, (X, y) in enumerate(dataloader):
            ...
            # Compute prediction error
            pred, decoded = self.model(X)
            # max_indices = pred.argmax(dim=1)
            # max_tensor = max_indices

            # y = y.squeeze(1)
            loss_reconst = self.loss_reconstruction(decoded, X)
            loss_classify = self.loss_fn(pred, y)
            loss = loss_reconst * 0.5 + loss_classify * 0.5
            # loss = self.cross_entropy_with_weights(pred, y)
            self.train_loss.append(loss.item())
            # self.scheduler.step(loss)
            # Backpropagation
            ...
            if (batch + 1) % verbose_step == 0:
                print(
                    f"Epoch [{self.epoch + 1}/{self.epochs}], Step [{batch + 1}/{n_total_steps}], Loss: {loss.item():.4f}, Learning Rate {self.optimizer.param_groups[0]['lr']}")
                # print(self.model.sl1.weight)
                # print(model.sl2.weight)
                # print(model.sl2.connections)
        ...
        with torch.no_grad():
            with alive_bar(total=len(dataloader), force_tty=True, title="Calculating Train accuracy:\t\t") as bar:
                for X, y in dataloader:
                    ...
                    # y = y.squeeze(1)
                    pred, decoded = self.model(X)
                    loss_reconst = self.loss_reconstruction(decoded, X).item()
                    loss_classify = self.loss_fn(pred, y).item()
                    loss = loss_reconst * 0.5 + loss_classify * 0.5
                    check_train_loss += loss
                    # check_train_loss += self.cross_entropy_with_weights(pred, y).item()
                    correct += (pred.argmax(1) == y).type(torch.float).sum().item()
        ...
        correct /= size
        print(f"Train Error: \t\tAccuracy: {(100 * correct):>0.2f}%, average train loss: \t\t{check_train_loss:>8f}")
        print(f"losses: \t {(100 * loss_reconst):>0.2f} \t {(100 * loss_classify):>0.2f} \t {(100 * loss):>0.2f} ")
    ...
    def evaluate(self, dataloader):
        ...
        correct /= size
        print(
            f"Evaluation Error: \tAccuracy: {(100 * correct):>0.2f}%, average evaluation loss: \t{self.eval_loss:>8f}")
        print(f"losses: \t {(100 * loss_reconst):>0.2f} \t {(100 * loss_classify):>0.2f} \t {(100 * loss):>0.2f} ")
        # metric calculation with torchmetrics
        bacc_all_batches = metric_bacc.compute()
        bprec_all_batches = metric_bprec.compute()
